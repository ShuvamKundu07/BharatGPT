...
const SideBar = ({isMenuOpen, setIsMenuOpen})=>{ 
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-linear-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 fixed ${!isMenuOpen && 'max-md:-translate-x-full'}`}>

      {/* Logo */}
      <img 
        src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark} 
        alt="" 
        className='w-full max-w-55 pl-5'
      />
      ...
      <div onClick={()=>{navigate('/credits');setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-xs text-shadow-gray-400'>Purchase credits to use BharatGPT</p>
        </div>
      </div>
      ...
      <div className='flex items-center gap-3 p-3 mt-4 border     border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
        ...
        {user && <img onClick={logout} src={assets.logout_icon} className='h-5 cursor-pointer block absolute bottom-5 right-5 z-50'/>}
      </div>

      <img onClick={()=>setIsMenuOpen(false)} src={assets.close_icon} className='absolute md:hidden top-3 right-3 w-5 h-5 cursor-pointer not-dark:invert' alt="" />
    </div>
  )
}
